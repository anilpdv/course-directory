...
class FileSystemService {
  ...
  private getItemName(item: File | Directory): string {
    // Handle both File and Directory objects
  ...
  private extractFolderName(uri: string): string {
    ...
    // Remove file:// prefix and trailing slashes
    ...
    // If it looks like a system path (contains dots like com.app.name),
    // try to find a better name
    if (name.includes('.') && name.split('.').length > 2) {
      // This might be an app identifier, look for a better name
  ...
  async scanCourses(): Promise<Course[]> {
    ...
    try {
      ...
      if (directVideos.length > 0) {
        // Treat selected folder as a single course with videos
      ...
      // Otherwise, scan subdirectories as courses
